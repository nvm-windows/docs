...
В этом режиме используются ссылки для определения активной системной версии `node.exe`. Ссылка входит в `PATH`. При выполнении [`nvm use`](../command/nvm/use) меняется _цель_ пути ссылки, а не сам путь. `PATH` не меняется.
