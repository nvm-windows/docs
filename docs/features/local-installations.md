...
This differs from the ordinary [download cache](./cache): `local_dir` **replaces** `.cache/versions` as the archive root for installs. With `local_install_only`, a miss never falls back to the network.

## Settings

| Option | Registry | Default | Effect |
|--------|----------|---------|--------|
| `local_dir` | `LocalInstallDir` | _(unset)_ | Directory of Node.js `.7z` archives used as the install source |
| `local_install_only` | `LocalInstallOnly` | `false` | Install only from that local source; fail if the archive is missing |
...
Place official Windows Node.js archives in `local_dir` (or the default version cache if you are not using `local_dir`):
...
On a connected machine you can populate a folder with [`nvm cache add`](../command/cache/add) and copy `.cache/versions` (or set that path as `local_dir` on the offline host).
...
1. Look for `node-v24.11.0-win-*.7z` under `local_dir` (or `.cache/versions` when `local_dir` is unset).
...
4. If missing and **`local_install_only=true`** → fail with `not found in local install directory` (no download).
...
- [Download Cache](./cache) — default `.cache/versions` store and `--cache` / `cache_downloads`
...
:::info[`air_gapped`]
The separate `air_gapped` preference controls offline license/JWKS behavior for Author mirrors. It does **not** by itself redirect Node.js installs to a local archive folder — use `local_dir` / `local_install_only` for that.
:::
