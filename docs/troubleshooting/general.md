...
| `Blocked by this computer's policy.` | Policy-managed setting/operation _(Certified Builds)_ | `nvm config list`; `nvm config get <key>` | Coordinate with IT policy owner; do not force local overrides |
...
## Certified Builds / fleet notes

- On Certified Builds fleets, Governance policy can intentionally block local mutation for specific settings.
