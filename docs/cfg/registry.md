...
|**Node.js download mirrors**|`MirrorNode`|**Overridable** (`node_mirror`). Ordered mirror list; first successful response wins. Author `*.author.io` mirrors need Governance licensing — see [Download Mirror & Firewall](../features/proxy).<br /><br />Default: `https://nodejs.org/dist`<br /><br />`REG_MULTI_SZ`|
...
|**Proxy URL**|`Proxy`|**Overridable** (`proxy`). Proxy for downloads. Basic and Bearer work on all certified editions. See [Download Mirror & Firewall](../features/proxy).<br /><br />`REG_SZ`|
